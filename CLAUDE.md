...
# DREADPULSE Project Conventions

이 프로젝트는 로그라이트 오토배틀러 **DREADPULSE**다. 기준 문서:

- 기획서(북극성): `docs/DREADPULSE_GDD.md`
- Phase 0 설계: `docs/superpowers/specs/2026-07-04-dreadpulse-phase0-design.md`

## 네이밍 (GDD 부록 A 용어사전 강제)

코드/데이터의 식별자는 반드시 GDD 부록 A의 영문 명칭을 쓴다:
`heart`, `pulse_interval`, `pulse`, `charge`, `wire`/`nerve`, `steam`, `ammo`,
`ichor`, `hull`, `corruption`, `graft`, `devour`, `compartment`, `hardpoint`.
새 용어가 필요하면 GDD 부록 A에 먼저 추가한 뒤 사용한다.

## 아키텍처 규칙

- `res://sim/`은 **순수 로직 계층**이다. Node/씬/Engine 싱글톤(시간, 입력, 렌더)을
  참조하지 않는다. 모든 클래스는 `RefCounted` 기반.
- 모든 확률은 주입된 시드의 `RandomNumberGenerator`만 사용한다. `randf()` 등
  전역 RNG 호출 금지 — 결정론(같은 빌드+시드 = 같은 결과)이 배치 검증의 전제다.
- `debug/`(시각화)와 `tests/`(배치 러너)는 sim이 방출하는 이벤트 스트림만 소비한다.
  sim 내부 상태를 직접 후벼 그리지 않는다.
- 부품 추가는 `sim/parts_catalog.gd`에 데이터 항목 추가로 해결한다. 부품별 서브클래스 금지.

## 검증

- 배치 검증: `godot --headless --script res://tests/run_batch.gd`
- 단위 검증: `godot --headless --script res://tests/run_unit.gd`
- 밸런스 수치는 전부 플레이스홀더. 수치 변경은 자유롭되 검증 지표(지수 곡선,
  촉수 추월, 불발률)가 리포트에서 깨지는지 확인할 것.
