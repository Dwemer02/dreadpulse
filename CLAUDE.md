...
- **가속/둔화는 배율이 고정이다** — 가속은 쿨타임 진행 ×2, 둔화는 ×0.5.
  ...
  가속과 둔화가 겹치면 남은 시간끼리 상쇄하고, **같은 종류는 합산이 아니라 큰 쪽으로
  갱신한다** (정지도 같다). 합산이면 1~2초짜리 작은 가속을 뿌리는 파츠 두 장만으로
  영구 가속이 되고, 그것을 전제로 설계된 파츠가 없다.
- **발동 횟수는 기본 무제한이다.** `active.fire_limit`을 명시한 파츠와 `drain_fires`를
  맞은 파츠만 유한해지며, 무제한 파츠가 처음 `drain_fires`를 맞으면 그 순간
  `DEFAULT_FIRE_LIMIT = 5`로 확정된다. **0이 되어도 파손되지 않는다** — 주기 발동만
  멈추고 슬롯에 남는다. 파괴는 `destroy_self`를 명시한 파츠만 한다. 소진된 채 살아
  있는 상태가 AT09의 대상이고 AT10의 사건이다.
- **Restore는 발동 횟수를 채워주지 않는다.** 성장(`growth`)은 유지하고 잔여 횟수는
  그대로 둔다. 되돌리려는 파츠는 자기 효과로 명시한다 (AT08).
...
- **파손된 파츠가 볼 수 있는 사건은 자기 자신의 `part_destroyed` 하나뿐이다.**
  그래서 "숙주가 실제 Destroy될 때 …" 형태의 AUGMENT는 **작동한다** (RD01·02·03·06).
  그 외의 사건은 전부 멈춘다 — "숙주가 파손된 뒤 매 초" 같은 효과는 여전히 조용히
  죽으므로 **관찰자 파츠**에 얹어야 한다.
- **재생은 부여마다 독립된 유한 효과다.** 부여 시점부터 2초마다 회복하고, 회복도
  이벤트도 부여별로 따로 난다. 출처 파츠가 파괴되면 그 출처의 남은 재생은 종료한다.
  재생 틱도 "실제 Repair"이므로 `repaired`를 낸다 (`source: "regen"`) — 단 하나의
  예외가 Viridia Core이며 `triggers_repair: false`로 데이터에 명시되어 있다.
...
- **파츠 파일은 아키타입 단위다** (`sim/data/parts/<faction>_<archetype>.json`).
  한 파일이 곧 하나의 순수 루프이고, 파츠마다 `archetype` 코드(RF/RC/RD/VC/VE/VG/
  AE/AH/AT)가 붙는다. 아키타입은 **설계상 주 용도이지 장착·증강 제한이 아니다.**
  Core 3종은 아키타입이 없다 — 90종 풀 밖이다.
