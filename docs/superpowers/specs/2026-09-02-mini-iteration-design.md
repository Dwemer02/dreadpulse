...
## 2. 결정 대기 — 이것부터 정해야 나머지가 정해진다

### 결정 1. Aeonic Core의 선체 재질 `energy_shield` (요청과 기존 계약의 충돌)
...
```
run/
  run_state.gd      런 1회의 상태: 인벤토리 · 보드 · 진행 위치 · 런 RNG
  inventory.gd      보유 파츠 인스턴스 목록과 슬롯 배치. 빌드 Dictionary를 만든다
  encounter.gd      노드 하나: 적 후보 2종 · Threat Profile · 결과
  salvage.gd        전투 후 3택1 후보 생성 (런 RNG)
  mini_iteration.gd 6노드 진행 관리. 상태 기계
  run_metrics.gd    §10 지표 집계. 이벤트 스트림 + 런 이벤트를 읽는다
  data/
    starters.json   팩션별 스타터 3종
    enemies.json    적 프리셋 6종 + Elite + Boss
    nodes.json      6노드 구성 (각 노드의 적 후보 2종)
```
...
| # | 단계 | 산출물 | 돌려보는 방법 |
|---|---|---|---|
| 1 | 선체 재질 3종 확정 (**결정 1**) | `HULL_MATERIALS` · CLAUDE.md · Core 3종 | 기존 테스트 + 상성 실측 |
...
| 7 | **UI** (**결정 2**) | Tune 화면 · 적 선택 · Salvage 3택1 | 사람이 플레이 |
