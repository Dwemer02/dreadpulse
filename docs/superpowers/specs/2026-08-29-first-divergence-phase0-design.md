...
> ## 일부 대체됨 (2026-08-30)
>
> 이 문서는 **작성 시점의 기록**이다. 본문은 고치지 않는다.
> 아래 항목은 이후 설계로 대체되었으므로, 충돌하면 대체 문서를 따른다.
>
> | 이 문서의 내용 | 대체 문서 |
> |---|---|
> | 파츠의 `roles: Array` 스키마, 슬롯 role `utility` | [슬롯 구조](2026-08-30-slot-structure-design.md) — `base_role: String` 단일 필드, `utility` → `system` |
> | 파츠 표의 키워드 배정 | [키워드 체계](2026-08-30-keyword-system-design.md) — 7층 구조, 공격/방어 타입 도입 |
>
> 본문에 남아 있는 `roles: ["utility", "flexible"]` · `utility_1` 등은
> **당시 스키마**이며 현재 코드와 다르다.
